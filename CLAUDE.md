...
services/api/                REST API server (Axum 0.7, 27 routes)
  src/handlers/                search, items, graph, typecheck, chat, artifacts, tasks, pg_query, health
services/mcp/                MCP protocol bridge (stdio + SSE transport on :3001, 14 tools)
```
...
**Chat (8 endpoints):** `/tools/chat`, `/tools/chat/stream`, sessions CRUD, fork, abort

**System:** `/health`, `/metrics`, `/api/snapshot`, `/api/ingestion/progress`, `/playground/*`
