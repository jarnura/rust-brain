...
### Available MCP tools
- **mcp_rustbrain_query_graph** — read-only Cypher queries against Neo4j. Max 4 queries. Verify callers, dependency ordering, trait completeness.
- **mcp_rustbrain_pg_query** — raw SQL against PostgreSQL *(to be built — Phase 0)*. Max 6 queries. Symbol signature lookup, name collision check.
...
- None. Planner has NO bash access.

### NOT available
- File reading (no bash cat/head/tail) — rely on CodeMap from Explorer
- File writing (no edit access)
- Semantic search (no mcp_rustbrain_search_code)
- Webfetch: DENIED
- Task dispatch: DENIED

### PostgreSQL queries (extracted_items + source_files)

**Symbol signature lookup:**
```sql
...
```sql
SELECT ei.fqn, ei.name, ei.item_type, sf.file_path, sf.crate_name
FROM extracted_items ei
JOIN source_files sf ON ei.source_file_id = sf.id
WHERE ei.name = $1;
```
...
- `DEPENDS_ON` (Crate→Crate) — requires DEPENDS_ON relationship (Phase 0). Use `cargo tree` output from CodeMap or Explorer as workaround.
- `HAS_METHOD` (Trait→Function) — requires HAS_METHOD relationship (Phase 0). Use CONTAINS relationship through Trait nodes as workaround.
...
2. Verify with mcp_rustbrain_query_graph (all callers must be in CodeMap).
...
1. Map all usage sites from CodeMap + mcp_rustbrain_query_graph.
...
4. Validate with mcp_rustbrain_query_graph: every caller of a modified pub fn must appear as a subsequent change.
---
