...
use qdrant_client::{PayloadValue, Point, QdrantClient, QdrantConfig, SearchRequest};
use std::collections::HashMap;
use std::sync::Arc;
use text_representation::{extract_doc_chunks, generate_text_representation, DocChunk};
...
impl EmbeddingService {
    ...
    pub fn new(config: EmbeddingConfig) -> Result<Self> {
        let ollama =
            OllamaClient::new(config.ollama.clone()).context("Failed to create Ollama client")?;

        let qdrant =
            QdrantClient::new(config.qdrant.clone()).context("Failed to create Qdrant client")?;
    ...
    pub async fn embed_item_with_docs(
        &self,
        item: &ParsedItem,
    ) -> Result<(EmbeddedItem, Vec<EmbeddedDoc>)> {
    ...
    pub async fn get_stats(&self) -> Result<EmbeddingStats> {
        let code_info = self
            .qdrant
            .get_collection_info(self.qdrant.code_collection())
            .await
            .context("Failed to get code collection info")?;

        let doc_info = self
            .qdrant
            .get_collection_info(self.qdrant.doc_collection())
            .await
    ...
    fn build_item_payload(&self, item: &ParsedItem, text: &str) -> HashMap<String, PayloadValue> {
        ...
        payload.insert(
            "item_type".to_string(),
            PayloadValue::from(item.item_type.as_str()),
        );
        payload.insert(
            "visibility".to_string(),
            PayloadValue::from(item.visibility.as_str()),
        );
        ...
        payload.insert(
            "signature".to_string(),
            PayloadValue::from(item.signature.as_str()),
        );
        ...
        payload.insert(
            ...
            PayloadValue::from(item.start_line as i64),
        );
        payload.insert(
            "end_line".to_string(),
            PayloadValue::from(item.end_line as i64),
        );
        ...
        if !item.generic_params.is_empty() {
            if let Ok(generics_json) = serde_json::to_string(&item.generic_params) {
                payload.insert(
                    ...
                    PayloadValue::from(generics_json),
                );
            }
        }
        ...
        if !item.where_clauses.is_empty() {
            ...
            let trait_bounds: Vec<String> = item
                .where_clauses
                .iter()
    ...
    fn build_doc_payload(&self, chunk: &DocChunk) -> HashMap<String, PayloadValue> {
        ...
        payload.insert(
            ...
            PayloadValue::from(chunk.source_fqn.as_str()),
        );
        payload.insert(
            "source_item_type".to_string(),
            PayloadValue::from(chunk.source_item_type.as_str()),
        );
        payload.insert(
            "chunk_index".to_string(),
            PayloadValue::from(chunk.chunk_index as i64),
        );
    ...
    fn from(result: qdrant_client::SearchResult) -> Self {
        ...
        SearchResult {
            ...
            signature: payload
                ...
                .unwrap_or_default(),
            doc_comment: payload.get("doc_comment").and_then(|v| match v {
                PayloadValue::String(s) => Some(s.clone()),
                _ => None,
            }),
        }
...
mod tests {
    ...
    fn test_build_item_payload() {
        ...
        assert!(
            matches!(payload.get("fqn"), Some(PayloadValue::String(s)) if s == "test_crate::module::test_fn")
        );
        assert!(matches!(payload.get("name"), Some(PayloadValue::String(s)) if s == "test_fn"));
        assert!(
            matches!(payload.get("item_type"), Some(PayloadValue::String(s)) if s == "function")
        );
        assert!(
            matches!(payload.get("crate_name"), Some(PayloadValue::String(s)) if s == "test_crate")
        );
    }
    ...
    fn test_build_item_payload_has_module_path() {
        ...
        assert!(
            matches!(payload.get("module_path"), Some(PayloadValue::String(s)) if s == "test_crate::module")
        );
        assert!(
            matches!(payload.get("signature"), Some(PayloadValue::String(s)) if s.contains("test_fn"))
        );
        assert!(matches!(
            payload.get("start_line"),
            Some(PayloadValue::Integer(10))
        ));
        assert!(matches!(
            payload.get("end_line"),
            Some(PayloadValue::Integer(15))
        ));
        assert!(matches!(
            payload.get("has_generics"),
            Some(PayloadValue::Boolean(false))
        ));
    }
    ...
    fn test_build_item_payload_with_generics() {
        ...
        assert!(matches!(
            payload.get("has_generics"),
            Some(PayloadValue::Boolean(true))
        ));
    ...
    fn test_build_doc_payload() {
        ...
        assert!(
            matches!(payload.get("source_fqn"), Some(PayloadValue::String(s)) if s == "my_crate::module::item")
        );
        assert!(
            matches!(payload.get("crate_name"), Some(PayloadValue::String(s)) if s == "my_crate")
        );
        assert!(matches!(
            payload.get("chunk_index"),
            Some(PayloadValue::Integer(0))
        ));
        assert!(
            matches!(payload.get("text"), Some(PayloadValue::String(s)) if s == "This is doc text")
        );
    }
