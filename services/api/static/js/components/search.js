...
class SearchPanel {
  ...
  async _doSearch() {
    ...
    try {
      const data    = await apiClient.aggregateSearch(query, { limit: 20 });
  ...
  _applyFilter() {
    ...
    const filtered = filter.kind
      ? this._lastResults.filter(r => {
          ...
          return k === filter.kind || k.startsWith(filter.kind);
        })
      : this._lastResults;
  ...
  _row(r) {
    ...
    const file     = r.file || r.source_file || '';
    ...
    const snippet  = this._includeSource && (r.snippet || r.source_snippet)
      ? `<pre class="sp-result__snippet"><code class="language-rust">${this._esc(r.snippet || r.source_snippet)}</code></pre>`
      : '';
