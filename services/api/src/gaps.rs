...
async fn check_semantic_search(state: &AppState) -> FeatureStatus {
        ...
        .await
    {
        Ok(resp) if resp.status().is_success() => {
            if let Ok(json) = resp.json::<serde_json::Value>().await {
                if let Some(count) = json.get("result")
        ...
        Ok(resp) => {
            details.push(format!("Qdrant collection not found (status: {})", resp.status()));
        ...
        .await
    {
        Ok(resp) if resp.status().is_success() => {
            if let Ok(json) = resp.json::<serde_json::Value>().await {
                let models: Vec<&str> = json.get("models")
                ...
                if has_model {
                    ...
                    details.push(format!("Embedding model '{}' not found. Available: {:?}", model_name, models));
                    status = if status == FeatureState::Working { FeatureState::Partial } else { status };
                }
            }
        }
        Ok(_) => {
...
fn generate_recommendations(features: &[FeatureStatus], data_quality: &DataQuality) -> Vec<Recommendation> {
    ...
    if let Some(f) = semantic {
        ...
        } else if f.status == FeatureState::Partial {
            if data_quality.qdrant_points == 0 {
                recommendations.push(Recommendation {
                    priority: "high".to_string(),
    ...
    if let Some(f) = callers {
        if f.status == FeatureState::Partial && !data_quality.has_call_graph {
            recommendations.push(Recommendation {
                priority: "high".to_string(),
                action: "Run indexer with --enable-call-graph to generate CALLS relationships".to_string(),
                impact: "Enables call graph analysis - find callers, callees, and trace execution paths".to_string(),
            });
        }
    }
    ...
    if data_quality.neo4j_nodes == 0 && data_quality.postgres_items > 0 {
        recommendations.push(Recommendation {
            ...
            impact: "Enables all graph-based queries (callers, trait impls, module tree)".to_string(),
        });
    }
    ...
    if data_quality.neo4j_nodes > 0 && data_quality.neo4j_relationships == 0 {
        recommendations.push(Recommendation {
            priority: "medium".to_string(),
            action: "Run relationship extraction phase of indexer (CALLS, IMPLEMENTS, CONTAINS)".to_string(),
    ...
    if data_quality.postgres_items > data_quality.qdrant_points * 2 {
        recommendations.push(Recommendation {
            ...
            action: "Re-run embedding generation to ensure all functions have vector embeddings".to_string(),
    ...
    recommendations.push(Recommendation {
        priority: "low".to_string(),
        action: "Consider adding macro expansion analysis for derive macro generated code".to_string(),
        impact: "Would capture auto-generated implementations (Debug, Clone, Serialize, etc.)".to_string(),
    });
    ...
    if data_quality.has_embeddings {
        recommendations.push(Recommendation {
            priority: "low".to_string(),
            action: "Extend embedding generation to structs, enums, and traits (not just functions)".to_string(),
...
mod tests {
    ...
    fn test_known_issues_count() {
        ...
        assert!(!issues.is_empty(), "Known issues database should not be empty");
        ...
        for issue in &issues {
            ...
            assert!(["critical", "high", "medium", "low"].contains(&issue.severity.as_str()),
                "Severity should be valid: {}", issue.severity);
        }
    }
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("\"status\":\"Working\"") || json.contains("\"status\":\"working\""), 
            "Should serialize feature state, got: {}", json);
        assert!(json.contains("\"neo4j_nodes\":100"), "Should serialize data quality");
    }
}
